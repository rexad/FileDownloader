...
using FileDownloader.Logic.ProtocolProvider;
    ...
    public class Downloader
    {
        ...
        public void WaitForConclusion()
        {
            if (!IsWorking())
            {
                ...
                {
                    mainThread.Join(/*TimeSpan.FromSeconds(1)*/1);
                }
            }
        ...
        private void StartDownloadThreadProc(object objSegmentCount)
        {
            ...
            int segmentCount = Math.Min((int)objSegmentCount, /*Settings.Default.MaxSegments*/10);
            ...
            do
            {
                ...
                {
                    lastError = ex;
                    if (currentTry < /*Settings.Default.MaxRetrie*/10)
                    {
                        SetState(DownloaderState.WaitingForReconnect);
                        Thread.Sleep(TimeSpan.FromSeconds(/*Settings.Default.RetryDelay*/10));
                    }
                    else
                    {
                        SetState(DownloaderState.NeedToPrepare);
        ...
        private void RestartDownload()
        {
            ...
            try
            {
                do
                {
                    ...
                    {
                        lastError = ex;
                        if (currentTry < 10/*Settings.Default.MaxRetries*/)
                        {
                            SetState(DownloaderState.WaitingForReconnect);
                            Thread.Sleep(TimeSpan.FromSeconds(10/*Settings.Default.RetryDelay*/));
                        }
                        else
                        {
                            return;
        ...
        private bool RestartFailedSegments()
        {
            ...
            for (int i = 0; i < this.Segments.Count; i++)
            {
                if (Segments[i].State == SegmentState.Error &&
                    ...
                    (/*Settings.Default.MaxRetries*/10 == 0 ||
                     Segments[i].CurrentTry < 10/*Settings.Default.MaxRetries*/))
                {
                    hasErrors = true;
                    TimeSpan ts = DateTime.Now - Segments[i].LastErrorDateTime;

                    if (ts.TotalSeconds >= /*Settings.Default.RetryDelay*/10)
                    {
                    ...
                    {
                        delay = Math.Max(delay, /*Settings.Default.RetryDelay*/10 * 1000 - ts.TotalMilliseconds);
                    }
                }
            }
            ...
            lock (segments)
            {
                for (int i = 0; i < this.segments.Count; i++)
                {
                    ...
                    if (oldSegment.State == SegmentState.Downloading &&
                        oldSegment.Left.TotalSeconds > /*Settings.Default.MinSegmentLeftToStartNewSegment*/10 &&
                        oldSegment.MissingTransfer / 2 >= /*Settings.Default.MinSegmentSize*/10)
                    {
